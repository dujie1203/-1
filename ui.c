...
#endif
void set_wifi_ip_text(const char * text)
{
    lv_label_set_text(wifi_ip_text, text);
}
///////////////////// ANIMATIONS ////////////////////
void muyushow_Animation(lv_obj_t *TargetObject, int delay)
{
...
void ui_event_muyuplay(lv_event_t *e)
{
    ...
    if (event_code == LV_EVENT_UP_CLICK || event_code == LV_EVENT_DOWN_CLICK)
    {
...
void ui_event_game(lv_event_t *e)
{
    ...
    if ((event_code == LV_EVENT_GESTURE && lv_indev_get_gesture_dir(lv_indev_get_act()) == LV_DIR_LEFT) ||
        (event_code == LV_EVENT_RIGHT_LONG_CLICK))
    {
        lv_indev_wait_release(lv_indev_get_act());
    ...
    if ((event_code == LV_EVENT_GESTURE && lv_indev_get_gesture_dir(lv_indev_get_act()) == LV_DIR_RIGHT) ||
        (event_code == LV_EVENT_LEFT_LONG_CLICK))
    {
        lv_indev_wait_release(lv_indev_get_act());
        _ui_screen_change(&ui_face, LV_SCR_LOAD_ANIM_NONE, 0, 0, &ui_face_screen_init);
        game_screen_flag = 0;
    ...
    if (event_code == LV_EVENT_UP_CLICK)
    {
        uint8_t key = LV_KEY_UP;
        lv_event_send(ui_2048, LV_EVENT_KEY, &key);
        game_screen_flag = 1;
    }
    if (event_code == LV_EVENT_DOWN_CLICK)
    {
        uint8_t key = LV_KEY_DOWN;
        lv_event_send(ui_2048, LV_EVENT_KEY, &key);
        game_screen_flag = 1;
    }
    if (event_code == LV_EVENT_LEFT_CLICK)
    {
        if (game_screen_flag)
        {
    ...
    if (event_code == LV_EVENT_RIGHT_CLICK)
    {
        if (game_screen_flag)
        {
