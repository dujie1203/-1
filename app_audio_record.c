...
static void audio_detect_task(void *pvParam)
{
    ...
    while (true)
    {
        ...
        {
            time_cnt ++;
            ESP_LOGI(TAG, "Volume %.2f",res->data_volume);
            if (res->data_volume < -35 && time_cnt>30) // 录音播放结束了
            {
                ESP_LOGI(TAG, "echo end");
                start_echo_flag = 0;
            }
            if(time_cnt >= 60)
            {
...
esp_err_t app_sr_start(void)
{
    ...
    */
    ...
    xTaskCreatePinnedToCore(app_stt_task, "audio_stt", 1024 * 4, NULL, 2, NULL, tskNO_AFFINITY);
    xTaskCreatePinnedToCore(audio_chat_task, "audio_chat", 1024 * 6, NULL, 1, NULL, 0);
    xTaskCreatePinnedToCore(audio_tts_task, "audio_tts", 1024 * 6, NULL, 6, NULL, tskNO_AFFINITY);
